...
func _ready() -> void:
	print("dialog_merchant: ", GameManager.instance.dialog_merchant.first_interaction_active)
	_randomize_items()
...
func _dialog_update() -> void:
	if GameManager.instance.dialog_merchant.first_interaction_active:
		dialog_label_text.text = GameManager.instance.dialog_merchant.first_interaction
		GameManager.instance.dialog_merchant.first_interaction_active = false
		return
	
	dialog_label_text.text = GameManager.instance.dialog_merchant.lines[randi_range(0, 1)]
